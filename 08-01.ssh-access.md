...
## 5. Forward a credential to an auto-created sandbox (optional)

If you let SSH **auto-create** a sandbox on connect, it starts with no daemon-held credentials, so a real agent would be unauthenticated. Your client already sends `SendEnv *`; the daemon then injects **only** the names on its `ssh.acceptEnv` allowlist (default-deny, and loader/exec vars like `PATH`, `LD_*`, `BASH_ENV` are hard-denied even if listed):

```bash
sbx settings set ssh.autoCreate true                       # create-on-connect (off by default)
sbx settings set ssh.acceptEnv '["OPENAI_API_KEY"]'        # allowlist what may be forwarded
sbx daemon stop && sbx daemon start                        # both are read at daemon startup

export OPENAI_API_KEY=sk-...        # in your shell
ssh fresh-demo.sbx                  # daemon creates "fresh-demo" and forwards the key for this session
```

Values are per-connection and **never stored at rest or logged**. For the durable, proxy-managed path, prefer `sbx secret set` (see [`03-02.secrets-injection.md`](./03-02.secrets-injection.md)) — shell-env forwarding is the lightweight, ephemeral alternative for SSH sessions.

## 6. Boundaries (current slice)

- **File transfer is SFTP, not `scp`.** Use `sftp codex-ssh.sbx`; `scp` is not separately wired.
- **Port forwarding is loopback-only.** `ssh -L` / `ssh -R` reach the sandbox's own loopback address; off-box targets are rejected. (To expose a service to your host, use `sbx ports … --publish` as in [`02-01.github-interaction.md`](./02-01.github-interaction.md).)
- **Auto-create has no auto-delete.** Connecting can create a sandbox, but nothing reaps it — clean up with `sbx rm`.
- **Login is enforced.** New SSH sessions require an active Docker login (`sbx login`); `sbx logout` blocks new connections.
...
> [!NOTE]
> SSH access is experimental and newer than the current published CLI reference, so there is **no dedicated official docs page yet**. The links below are the closest official material; expect a dedicated page as the feature ships.
